...
$domain, $ou, $groupName, $userName, $email, $emailServer, $emailTo, $emailFrom = $null, $null, $null, $null, $null, $null, $null, $null

#getting user input
$domain = Read-Host -Prompt "Enter target domain (or leave blank to query current domain)"
$ou = Read-Host -Prompt "Enter OU -- OU=<OU_Name>,OU=OU_Name,DC=Your,DC=Company,DC=Com -- (or leave blank to query entire domain)"
...
} elseif ($ou.Length -gt 0) {
    $users = Get-ADUser -Server $domain -SearchBase $ou -Filter {(Enabled -eq $true) -and (PasswordNeverExpires -eq $false)} –Properties "msDS-UserPasswordExpiryTimeComputed", *
} else {
...
#if a group name was entered and a user name was not, this will filter the output to only include users that are a member of the group
if (($groupName.Length -gt 0) -and ($userName.Length -eq 0)) {
    $output = $output | Where-Object {$_.MemberOf -contains $groupName}
}
